...
class DoctorSettingsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnUpdateName.setOnClickListener {
            ...
            val email = sessionManager.getEmail() ?: return@setOnClickListener
        ...
        binding.btnChangePassword.setOnClickListener {
            ...
            if (current.isEmpty() || newPass.isEmpty() || confirm.isEmpty()) return@setOnClickListener
            if (newPass != confirm) {
    ...
    private fun updateName(email: String, name: String) {
        val token = sessionManager.getBearerToken()
        lifecycleScope.launch {
            try {
                val r = RetrofitClient.instance.updateName(token, email, UpdateNameRequest(name))
                if (r.isSuccessful) {
    ...
    private fun changePassword(current: String, newPass: String) {
        ...
        lifecycleScope.launch {
            try {
                val r = RetrofitClient.instance.changePassword(token, email, ChangePasswordRequest(current, newPass))
                if (r.isSuccessful) {
